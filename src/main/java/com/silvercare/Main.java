...
import com.silvercare.view.MainView;
 ...
 *
 * 編譯指令（Mac/Linux）：
 *   javac -cp "lib/*" -d out -encoding UTF-8 src/main/java/com/silvercare/**\/*.java
 *
 * 執行指令：
 *   java -cp "out:lib/*" com.silvercare.Main         (Mac/Linux)
 *   java -cp "out;lib\*" com.silvercare.Main          (Windows)
 *
 * 或直接用：
 *   ./run.sh       (Mac/Linux)
 *   run.bat        (Windows)
...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 原始 MainView mainView = new MainView(scanner);
            mainView.start();
